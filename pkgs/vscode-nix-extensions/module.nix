...
with lib; {
    ...
    paths = mkOption {
      type = with types;
        listOf (oneOf [
          ...
          (submodule {
            options = {
              from = mkOption { type = oneOf [ package pathInStore ]; };
      ...
      type = with types;
        attrsOf (submodule ({ name, ... }: {
          options = {
            ...
            path = mkOption {
              type = with types; nullOr (oneOf [ package pathInStore ]);
      ...
      type = with types;
        attrsOf (submodule ({ name, ... }: {
          options = {
            ...
            uiTheme =
              mkOption { type = with types; enum [ "vs-light" "vs-dark" ]; };
          };
        }));
      default = { };
    };
      ...
      type = with types;
        attrsOf (submodule ({ name, ... }@self: {
          options = {
            ...
            exec = mkOption {
              type = with types; nullOr (oneOf [ str package ]);
              default = null;
  ...
  config = {
    ...
    extraConfig = {
      inherit (config) name version description publisher;
      engines.vscode = "^1.81.1";
      categories = [ ];
      main = "./extension.js";
      nixExtension = { inherit (config) commands; };
      contributes = {
        ...
        themes =
          map (x: x // { path = ".${x.path}"; }) (attrValues config.themes);

        iconThemes =
          map (x: x // { path = ".${x.path}"; }) (attrValues config.iconThemes);
      };
    };
  };
}
