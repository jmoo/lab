...
let
  ...
  # Home-manager karabiner configuration (was modules/karabiner.nix, with the
  # internal option namespace moved off of `lab`).
  home =
    {
      pkgs,
      lib,
    ...
    in
    {
      ...
      config.karabiner.profiles.default.name = mkDefault "Home manager profile";
      config.karabiner.profiles.default.selected = mkDefault true;

      config.karabiner.settings = {
        profiles = attrValues config.karabiner.profiles;
      };

      config.karabiner.file = pkgs.writeTextFile {
        name = "karabiner.json";
        text = toJSON config.karabiner.settings;
      };

      config.home.file.karabiner-json = mkIf config.karabiner.enable {
        executable = false;
        source = config.karabiner.file;
...
in
{
  options.lab.hosts = mkHostModule (
    { config, ... }:
    {
      options.karabiner.enable = mkEnableOption "karabiner home-manager configuration";

      config = mkIf config.karabiner.enable (homeDarwin {
        imports = [ home ];
