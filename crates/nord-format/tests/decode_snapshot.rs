...
//!
//! Byte-exact round-trip cannot catch a wrong bit range: the backing word is preserved
//! verbatim, so a field reading the neighbouring bits still writes the file back
...
/// offsets is what is under test, and a snapshot compared against the numbers it came from
/// would pin nothing. Comes out when `OrganPanel` becomes a `#[bitpanel]` and [`packed`]
/// covers it.
struct Bytes {
    model: OrganModel,
...
///
/// Hand-written because `OrganPanel` is still a verbatim byte block with methods over it,
/// so there is no field metadata to walk — it is the one part of a program the macro
/// cannot describe, and the part with bits still undecoded, so leaving it out would leave
