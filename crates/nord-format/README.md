...
| `ns3f`/`ns3l` Stage 3 program | ✅ | ✅ | **Every documented parameter placed, on both panels** — 588 fields. The byte maps document one panel; the body holds two, the second repeating the first 263 bytes on, found from the corpus and enforced by a test. Values are named where the maps enumerate them and raw where they defer to the viewer's sources. Not confirmed on hardware. |
