...
| `ns3f`/`ns3l` Stage 3 program | ✅ | ✅ | **Every documented parameter placed** — globals, the organ's two presets and their drawbars, piano, synth, extern and the effects chain. Values are named where the byte maps enumerate them and raw where they defer to the viewer's sources. Not confirmed on hardware. |
| `ns2p`/`ns2l` Stage 2 program | ✅ | ✅ | The same, over the Stage 2's map: B3/Vox/Farfisa drawbar banks, piano, synth, extern, effects. |
