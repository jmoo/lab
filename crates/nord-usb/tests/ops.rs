...
fn move_reproduces_the_capture() {
    ...
    let mut t = ReplayTransport::new(wrap(vec![
        step(Out, "000000220000000c0000000a00000018000000070000000c000000060000000f4a55"),
        step(In, "000000260000000c0000000a0000001900000000000000070000000c000000060000000f7197"),
    ]));
    block_on(async {
        let mut s = Session::open(&mut t, ObjectClass::Program).await.unwrap().allow_destructive_writes();
        op::move_object(&mut s, Location::from_user(8, 13), Location::from_user(7, 16)).await.unwrap();
...
fn delete_reproduces_the_capture() {
    use Direction::{In, Out};
    let mut t = ReplayTransport::new(wrap(vec![
        step(Out, "000000240000000600000001000000060000000000000b44656c6574696e672e2e2e7394"),
        step(Out, "0000001a0000000c0000000a000000140000000600000031741e"),
        step(In, "0000001e0000000c0000000a0000001500000000000000060000003184b4"),
    ]));
    block_on(async {
        let mut s = Session::open(&mut t, ObjectClass::Program).await.unwrap().allow_destructive_writes();
        op::delete(&mut s, Location::from_user(7, 50)).await.unwrap();
        s.commit().await.unwrap();
    });
    assert!(t.is_exhausted(), "did not consume the whole exchange");
}
...
fn rename_reproduces_the_capture() {
    use Direction::{In, Out};
    let mut t = ReplayTransport::new(wrap(vec![
        step(Out, "000000210000000c0000000a0000001c000000050000000c00000003666f6f0d53"),
        step(In, "0000001e0000000c0000000a0000001d00000000000000050000000c86c2"),
    ]));
    block_on(async {
        let mut s = Session::open(&mut t, ObjectClass::Program).await.unwrap().allow_destructive_writes();
        op::rename(&mut s, Location::from_user(6, 13), "foo").await.unwrap();
        s.commit().await.unwrap();
    });
    assert!(t.is_exhausted(), "did not consume the whole exchange");
}
...
fn duplicate_reproduces_the_capture() {
    use Direction::{In, Out};
    let mut t = ReplayTransport::new(wrap(vec![
        step(Out, "000000220000000c0000000a000000160000000600000001000000060000000265f4"),
        step(In, "000000260000000c0000000a000000170000000000000006000000010000000600000002a86a"),
    ]));
    block_on(async {
        let mut s = Session::open(&mut t, ObjectClass::Program).await.unwrap().allow_destructive_writes();
        op::duplicate(&mut s, Location::from_user(7, 2), Location::from_user(7, 3)).await.unwrap();
        s.commit().await.unwrap();
    });
    assert!(t.is_exhausted(), "did not consume the whole exchange");
}
...
fn select_reproduces_the_capture() {
    ...
    let mut t = ReplayTransport::new(wrap(vec![
        ...
        step(In, "0000001e0000000c0000000a0000003000000000000000010000000b19df"),
    ]));
    block_on(async {
        let mut s = Session::open(&mut t, ObjectClass::Program).await.unwrap();
        op::select(&mut s, Location::from_user(2, 12)).await.unwrap();
        s.commit().await.unwrap();
    });
    assert!(t.is_exhausted(), "did not consume the whole exchange");
}
...
fn dependencies_decode_the_capture() {
    ...
    let deps = block_on(async {
        let mut s = Session::open(&mut t, ObjectClass::Program).await.unwrap();
        let deps = op::dependencies(&mut s, Location::from_user(7, 3)).await.unwrap();
        s.commit().await.unwrap();
        deps
    });
